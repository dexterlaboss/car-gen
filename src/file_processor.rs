    ...
    S: FileStorage + Send + Sync,
{
    ...
    pub async fn process_file(&self, file_path: &str) -> Result<()> {
        ...
        if let Some(peak_mem) = get_peak_memory_bytes() {
            info!("Peak memory usage: {}", format_bytes(peak_mem));
        }

        // Finalize after processing the file
        self.block_processor.finalize_blocks().await?;
